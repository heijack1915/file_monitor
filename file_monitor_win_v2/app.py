...
# 依赖检查必须在 Flask import 之前：第一次运行时 Flask 可能尚未安装
...
# ============ Flask 应用 ============
app = Flask(__name__)
# Flask 3.x 使用 app.json.ensure_ascii，兼容 2.x 的 JSON_AS_ASCII 已移除
...
def _handle_delete_async(path: str):
    """文件删除事件：延迟后推送，跳过 psutil（文件已消失）"""
    time.sleep(0.3)
    proc = _resolve_proc(path, try_psutil=False)
    _push_event("delete", path, proc)
